...
import { deleteDirectory, restorePackageJson, runCommandUntil } from '../../util/utils';
...
export default async function electronBuildExecutor(
  ...
  const { hostProject, hostProjectRoot, mainOutputFilename, mainOutputPath, author, description  } =
    options;
...
===============================
  ...
  parsedPackageJson.author = options.author;
  parsedPackageJson.description = options.description;
...
`);
  ...
  const resolveConfigFile = join(hostProjectRoot, 'electron', 'electron-builder.yml');
  const commandLine = `${getPackageManagerCommand().exec} electron-builder --config=${resolveConfigFile}`
